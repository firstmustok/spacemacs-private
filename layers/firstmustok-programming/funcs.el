...
(defun firstmustok/comment-box (b e)
...
(defun firstmustok/annotate-todo ()
...
(defun firstmustok/js2-which-function ()
...
(defun firstmustok/run-current-file ()
  ...
  (let* (
    ...
    (if (string-equal ξfSuffix "el") ; special case for emacs lisp
      ...
      (if ξprog-name
          (progn
            (message "Running…")
            (async-shell-command ξcmd-str "*firstmustok/run-current-file output*"))
...
(defun firstmustok/load-yasnippet ()
  (interactive)
