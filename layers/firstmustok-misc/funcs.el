;;; funcs.el --- firstmustok Layer packages File for Spacemacs
;;
;; Copyright (c) 2015-2016 firstmustok 
;;
...
(defun firstmustok/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun firstmustok/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun firstmustok/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun firstmustok/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun firstmustok/load-my-layout ()
  (interactive)
  (persp-load-state-from-file (concat persp-save-dir "zilong")))

(defun firstmustok/save-my-layout ()
  (interactive)
...
(defun firstmustok/indent-region(numSpaces)
  (progn
...
(defun firstmustok/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (firstmustok/indent-region 4)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun firstmustok/untab-region (N)
  (interactive "p")
  (firstmustok/indent-region -4))

(defun firstmustok/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'firstmustok/tab-region)
  (local-set-key (kbd "<S-tab>") 'firstmustok/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'firstmustok/hack-tab-key)
...
(defun firstmustok/helm-hotspots ()
  "helm interface to my hotspots, which includes my locations,
org-files and bookmarks"
  (interactive)
  (helm :buffer "*helm: utities*"
        :sources `(,(firstmustok//hotspots-sources))))

(defun firstmustok//hotspots-sources ()
  ...
  `((name . "Mail and News")
    (candidates . (("Calendar" . (lambda ()  (browse-url "https://www.google.com/calendar/render")))
                   ...
                   ("Run current flie" . (lambda () (firstmustok/run-current-file)))
...
(defun firstmustok/now ()
  "Insert string for the current time formatted like '2:34 PM'."
  (interactive)                 ; permit invocation in minibuffer
  (insert (format-time-string "%D %-I:%M %p")))

(defun firstmustok/today ()
...
(defun firstmustok/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun firstmustok/open-file-with-projectile-or-counsel-git ()
  (interactive)
  (if (firstmustok/git-project-root)
      (counsel-git)
...
(defun firstmustok/growl-notification (title message &optional sticky)
...
(defun firstmustok/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (firstmustok/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun firstmustok/goto-match-paren (arg)
...
(defun firstmustok/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun firstmustok/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun firstmustok/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (firstmustok/retrieve-chrome-current-tab-url)))

(defun firstmustok/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun firstmustok/single-lines-only ()
...
(defun firstmustok/named-term (name)
  (interactive "sName: ")
  (ansi-term "/bin/zsh" name))


(defun firstmustok/ash-term-hooks ()
...
(defalias 'tt 'firstmustok/terminal)
...
(defun firstmustok/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun firstmustok/evil-quick-replace (beg end )
  (interactive "r")
...
(defun firstmustok/git-project-root ()
...
(defun firstmustok/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (firstmustok/git-project-root)
                default-directory))
...
(defun firstmustok/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
...
(defun my-erc-hook (match-type nick message)
  ...
  (unless (posix-string-match "^\\** *Users on #" message)
    (firstmustok/growl-notification
...
(defun my-open-file-in-external-app (file)
  ...
  (interactive)
  (let ((default-directory (firstmustok/git-project-root))
        (file-path file))
    (if file-path
        (cond
...
(defun firstmustok/magit-visit-pull-request ()
...
(defun firstmustok/markdown-to-html ()
  (interactive)
...
(defun firstmustok/search-in-fireball ()
  (interactive)
  (helm-do-ag (expand-file-name "~/Github/fireball/")))


(defun firstmustok/show-current-buffer-major-mode ()
  (interactive)
  (describe-variable 'major-mode))

(defun firstmustok/counsel-imenu ()
  (interactive)
  (counsel-imenu)
  (evil-set-jump))
...
(defun firstmustok/add-node-modules-path ()
